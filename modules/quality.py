    ...
    for line in s.split("\n"):
        ...
        if m:
            ...
            if ":" in line or "：" in line:
                ...
                label = head.strip().upper()[:1]
        if label and len(label) == 1 and label in list("ABCDEFGHIJKLMNOPQRSTUVWXYZ"):
            opts.add(label)
    return opts
...
def assess_exercises(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        if t == "选择题":
            ...
            if ans and ans.upper() not in opts:
                s -= 30
                f.append(_flag("Error", "ANS_NOT_IN_OPTS", "答案不在选项中"))
        elif t == "判断题":
            valid = {"TRUE", "FALSE", "T", "F", "是", "否", "对", "错"}
...
def _is_garbled(text: str) -> bool:
    if not text:
        return False
    s = str(text)
    allowed = re.sub(r"[\w\u4e00-\u9fa5\s.,;，。；？！:：\-\(\)\[\]/]", "", s)
    ratio = len(allowed) / max(1, len(s))
    return ratio > 0.3
